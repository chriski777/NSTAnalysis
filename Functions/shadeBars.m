function [errorPatch,dataLine] = shadeBars(varargin)
...
%                         t = [-5:0.05:5];
%                         Y = sin(t);
%                         E = 0.4*ones(1,length(t));
%                         [errorPatch,dataLine] = shadeBars(t,Y,E);
%                         xlabel('time');
%                         ylabel('Amplitude');
...
errorPatch = patch(Xcoords,Ycoords,color,'linestyle','none','linewidth',1,'faceAlpha',alpha);% draw error bar patch
% set 'errorPatch' style
